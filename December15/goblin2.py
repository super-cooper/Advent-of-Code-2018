...
reset_hp, reset_board = copy.deepcopy(hp), copy.deepcopy(board)
targets, power, rounds = [], 4, 0
...
while True:
    ...
    for i, j in sorted((k, m) for m in range(len(board[0])) for k in range(len(board)) if board[k][m] in ('E', 'G')):
        ...
        if len(targets) == 0:
            print(
                rounds * sum(hp[(i, j)] for j in range(len(board[0])) for i in range(len(board)) if board[i][j] == 'E'))
            sys.exit()
        ...
        if len(adj) > 0:
            ...
            hp[victim] -= 3 if board[i][j] == 'G' else power
            if hp[victim] <= 0:
                if board[victim[0]][victim[1]] == 'E':
                    board, hp = copy.deepcopy(reset_board), copy.deepcopy(reset_hp)
                    rounds = -1
                    power += 1
                    break
