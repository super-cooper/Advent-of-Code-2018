...
print(min(enumerate(max(x + i * vx for (x, _, vx, __) in data) - min(x + i * vx for (x, _, vx, __) in data)
                    + max(y + i * vy for (_, y, __, vy) in data) - min(y + i * vy for (_, y, __, vy) in data)
                    for i in range(50_000)), key=lambda x: x[1])[0])
